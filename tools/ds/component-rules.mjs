...
const TABLE_RULES = {
  ...
  allowed: [
    ...
    'Менять ширину колонки: ячейка тянется по колонке, поэтому ширина задаётся колонкой.',
    'Менять ширину отдельной колонки под содержимое — это нормальная работа с таблицей.',
  ...
  forbidden: [
    ...
    'Менять высоту ячейки: высота ряда задана размером ячейки, ширина — колонкой.',
  ...
  checks: [
    ...
    'Ширина задана колонкой, а ячейка растянута по ней.',
...
/**
 * Правила для наборов ячеек.
 *
 * Ячейка не существует сама по себе: её место — колонка внутри таблицы.
 * Ширина приходит от колонки, поэтому ячейка растянута по родителю.
 * Высота задана размером ячейки и менять её нельзя — иначе ряды разъедутся.
 */
const CELL_RULES = {
  ...
  use: [
    ...
    'Ширина приходит от колонки: ячейка растянута по родителю, отдельно её задавать не нужно.',
    'Вид ячейки выбирается по содержимому: текст, текст с бейджем, статус, действие.',
    'Текст пишется в текстовый узел внутри ячейки.',
    'Статус — готовым бейджем, а не покрашенным текстом.'
  ],
  avoid: [
    'Не брать ячейку отдельно от строки.',
    'Не собирать ячейку из фреймов, если подходит готовый вариант.',
    'Не подгонять ширину ячейки числом: ширину задаёт колонка.'
  ],
  allowed: [
    'Менять текст внутри ячейки.',
    'Выбирать вариант по содержимому.',
    'Ставить бейдж статуса или иконку действия.',
    'Растягивать по колонке — это задано заранее.'
  ],
  forbidden: [
    'Менять высоту ячейки: высота задана её размером, иначе ряды разъедутся.',
    'Задавать ширину ячейке напрямую вместо колонки.',
    'Вставлять текст во фрейм ячейки, минуя текстовый узел.',
    'Оставлять заготовку вида «Text» или «Column Name» в готовом макете.',
    'Разбирать ячейку на части: замена — свапом варианта, а не пересборкой.'
  ],
  checks: [
    'Ячейка стоит в строке таблицы, а не сама по себе.',
    'Ширина пришла от колонки.',
    'Высота не менялась.',
    'В ячейке нет заготовки вида «Text».',
    'Статус — бейджем из набора.'
  ]
}

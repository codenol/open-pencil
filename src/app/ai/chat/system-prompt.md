...
- A slot is a container marked as one. `get_node` reports it: the node carries `slot: true`, and its parent lists `slots`. Do not guess a slot by its name — read the mark. A slot is filled with a finished block, not with loose nodes.
  ...
  - Instead: (1) create a component for the block you are building, named for what it is, for example `Dashboard PostgreSQL`; (2) fill that component — the content is saved because it belongs to a component, not to an override; (3) swap the slot's instance onto your component with `swap_component`, or drop your component's instance into the slot if it is a plain frame.

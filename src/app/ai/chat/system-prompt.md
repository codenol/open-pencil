...
- A slot is a container marked as one. `get_node` reports it: the node carries `slot: true`, and its parent lists `slots`. Do not guess a slot by its name — read the mark. A slot is filled with a finished block, not with loose nodes.
  - When the layout you were asked to fill is an instance (`Layout`, `Sidebar`, a page shell), the slot inside is where the block goes.
  - Do not add nodes straight into the instance: they show on screen but are not written to the file — the work is lost after save. Only text and fill overrides of nodes that already exist inside are stored.
  - Two ways to fill a slot, and the first is the usual one:
    1. **Fill the slot in the master.** Find the slot inside the *component* (`Layout`), not inside an instance, and render the block straight into it. The content belongs to the component, so it saves, and every instance of `Layout` shows it at once. This is what you normally want: the dashboard lives in the page shell, the page shell is a component.
    2. **Build a separate component and swap.** When the block is reusable on its own — a card that also appears elsewhere — make a component for it, fill that component, then put an instance of it into the slot with `render` (`<Instance of="..." />`) or `swap_component`. Do not write nodes into the slot of an instance for this: those are not saved.
  ...
  - Filling a slot is an ordinary `render` with the slot's id as the target: the renderer accepts a slot as a parent whether it sits in a component or in an instance. There is no separate slot tool to look for.
...
- Masters of the design system belong to the library, not to the task. Read them freely; do not reshape them to make one layout fit.
